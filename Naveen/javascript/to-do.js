...
var allItems = ( function() {
    ...
    return {
        ...
        store : function() {
            if(jsonStructure.listItems.length==0) {
                window.alert('Nothing in the List to store!');
            }
            else {
...
document.getElementById('json-List').addEventListener('click',function() {
    if(window.confirm('Do you want append all the list items from the external link to your current list ? ')) {
        retrieveList();
        document.getElementById('json-List').classList.add('dont-display');
    }
